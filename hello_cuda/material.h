...
public:
    __device__ virtual bool scatter(const ray& r_in, const hit_record& rec, vec3& attenuation,
      ray& scattered, float& pdf, curandState* local_rand_state) const = 0;
...
public:
    ...
    __device__ lambertian(texture_* a) : albedo(a) {}
    __device__ virtual bool scatter(const ray& r_in, const hit_record& rec, vec3& attenuation,
      ray& scattered, float& pdf, curandState* local_rand_state) const {
...
public:
  __device__ diffuse_light(texture_* a) : emit(a) {}
  __device__ diffuse_light(vec3 c) : emit(new solid_color(c)) {}

  __device__ virtual bool scatter(
    const ray& r_in, const hit_record& rec, vec3& attenuation,
    ray& scattered, float& pdf, curandState* local_rand_state
  ) const override {
    return false;
  }
